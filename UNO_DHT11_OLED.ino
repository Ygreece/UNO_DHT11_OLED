...
const float TEMPERATURE_HYSTERESIS_C = 1.0;
...
bool sensorValid = false, hasValidReading = false;
...
void updateAlarmLevel(float temperature) {
  if (alarmLevel == CRITICAL) {
    if (temperature < CRITICAL_TEMPERATURE_C - TEMPERATURE_HYSTERESIS_C) alarmLevel = WARNING;
    return;
  }
  if (alarmLevel == WARNING) {
    if (temperature >= CRITICAL_TEMPERATURE_C) alarmLevel = CRITICAL;
    else if (temperature < WARNING_TEMPERATURE_C - TEMPERATURE_HYSTERESIS_C) alarmLevel = NORMAL;
    return;
  }
  if (temperature >= CRITICAL_TEMPERATURE_C) alarmLevel = CRITICAL;
  else if (temperature >= WARNING_TEMPERATURE_C) alarmLevel = WARNING;
}
void updateAlarmOutputs() {
  ...
  if (alarmLevel == CRITICAL) {
    digitalWrite(STATUS_LED_PIN, HIGH); tone(BUZZER_PIN, 1000);
...
void showControlPage() {
  display.clearDisplay(); display.setTextColor(SSD1306_WHITE); display.setTextSize(1);
  display.setCursor(0, 0); display.println(F("P2 CONTROL"));
  display.print(F("Alarm: ")); display.println(alarmName());
  display.print(F("Fan: ")); display.print(fanModeName()); display.print(F(" ")); display.println(fanShouldRun() ? F("RUN") : F("STOP"));
  display.print(F("Warn/Crit: ")); display.print(WARNING_TEMPERATURE_C, 0); display.print(F("/")); display.print(CRITICAL_TEMPERATURE_C, 0); display.println(F("C"));
  display.print(F("Hyst: ")); display.print(TEMPERATURE_HYSTERESIS_C, 1); display.println(F("C")); display.display();
}
void showHelpPage() {
  display.clearDisplay(); display.setTextColor(SSD1306_WHITE); display.setTextSize(1); display.setCursor(0, 0);
  display.println(F("P3 BLUETOOTH")); display.println(F("STATUS")); display.println(F("PAGE 0/1/2"));
...
void sendPhoneReport() {
  ...
  Serial.print(F("Status: ")); Serial.println(alarmName());
  Serial.println(alarmLevel == NORMAL ? F("LED: OFF") : F("LED: ON"));
  Serial.println(alarmLevel == NORMAL ? F("Buzzer: OFF") : F("Buzzer: ON"));
...
void handleCommand() {
  ...
  else if (commandEquals("HELP")) Serial.println(F("OK STATUS | PAGE 0/1/2 | FAN AUTO/ON/OFF | HELP"));
